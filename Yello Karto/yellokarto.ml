...
let check_button_separate_colors =
       ...
       ~active:false
       ~packing:vboxOption1#add () in
         button#connect#clicked (fun () -> get_filter_separate (); set_filter ())
...
let check_button_grille =
       ...
       ~active:false
      ~packing:vboxOption1#add () in
        button#connect#clicked (fun () -> get_filter_grill (); set_filter ())
...
let display_button =
    let button = GButton.button
    ~label:"go"
    ~packing:vboxOption2#add () in
   ignore (button#connect#clicked (fun () -> (if(!img_not_empty) then
