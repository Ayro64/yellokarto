OCAMLFLAGS = -I +sdl -I +lablgtk2 -I +lablGL
OCAMLLD = sdl.cmxa sdlloader.cmxa bigarray.cmxa unix.cmxa lablgl.cmxa lablglut.cmxa lablgtk.cmxa lablgtkgl.cmxa -ccopt -L.
OCAMLOPT = ocamlopt

program: libgtkgl-2.0.so traitement_image.ml modelisation.ml moteur3D.ml yellokarto.ml
	${OCAMLOPT} ${OCAMLFLAGS} ${OCAMLLD} -o yellokarto traitement_image.ml modelisation.ml moteur3D.ml yellokarto.ml
	rm -f *~ *.o *.cm?

libgtkgl-2.0.so:
	-ln -s /usr/lib64/libgtkgl-2.0.so.1 libgtkgl-2.0.so

clean:
	rm -f *~ *.o *.cm? 
